...
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/${GIT_USER}/${PERMISSON}/main/ip | grep $IP_VPS | awk '{print $2}')
echo "$username" >/usr/bin/user

# Save Expired Detail in github to /usr/bin/e
expx=$(curl https://raw.githubusercontent.com/${GIT_USER}/${PERMISSON}/main/ip | grep $IP_VPS | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
function gasgas() {
# Status Check
Exp1=$(curl https://raw.githubusercontent.com/${GIT_USER}/${PERMISSON}/main/ip | grep $IP_VPS | awk '{print $4}')
...
print_message "Delete HAProxy, Nginx, xray configurations"
rm -rf /etc/haproxy/haproxy.cfg /etc/nginx/conf.d/xray.conf /etc/nginx/nginx.conf
...
install_Fail2ban() {
    ...
    systemctl restart fail2ban
        mkdir -p /usr/local/ddos /usr/sbin
...
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/${GIT_USER}/${PERMISSON}/main/ip | grep $IP_VPS | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/${GIT_USER}/${PERMISSON}/main/ip | grep $IP_VPS | awk '{print $3}')
